...
void FSM() {
  ...
  switch (state) {
    ...
    case DECODE:
      ...
      arch_state.control.ALUOp =
          0;  // add - TODO ??? optimistically compute branch addr
      // update state
...
void execute() {
      ...
      switch (IR_meta->function) {
        ...
        case SLT:
          next_pipe_regs->ALUOut =
              alu_opA < alu_opB ? 1 : 0;  // TODO check this
          break;
        default:
  ...
  // PC calculation
  const int jmp =
      ((curr_pipe_regs->pc >> 28) << 28) + (IR_meta->jmp_offset << 2);
  switch (control->PCSource) {
    ...
    case 1:  // BRANCH_COMPL
      next_pipe_regs->pc =
          curr_pipe_regs->ALUOut;  // optimistically computed in DECODE
      break;
    case 2:  // JUMP_COMPL
  ...
  if (arch_state.control.RegWrite) {
    ...
    if (write_reg_id > 0) {
      ...
      printf("Reg $%u = %d (from current %s)\n", write_reg_id, write_data,
             arch_state.control.MemtoReg ? "MDR" : "ALUOut");
    } else {
  ...
  switch (IR_meta->opcode) {
      ...
      switch (IR_meta->function) {
        case ADD:
          printf("Executing ADD(%d), $%u = $%u + $%u (function: %u) \n",
                 IR_meta->opcode, IR_meta->reg_11_15, IR_meta->reg_21_25,
                 IR_meta->reg_16_20, IR_meta->function);
          break;
        case SLT:
    ...
    case LW:
      printf("Executing LW(%d), $%u = %d($%u) (%u) \n", IR_meta->opcode,
             ...
             IR_meta->immediate + IR_meta->reg_21_25);
      break;
    case SW:
    ...
    case J:
      printf("Executing J(%d), $pc = %u", IR_meta->opcode,
             ((arch_state.curr_pipe_regs.pc >> 28) << 28) +
                 (IR_meta->jmp_offset << 2));
      break;
    case EOP:
